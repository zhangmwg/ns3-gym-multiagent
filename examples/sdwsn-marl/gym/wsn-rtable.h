...
public:
   ...
   * \param dst destination address （dst 为查找的 IP 地址）
   * \param rt entry with destination address dst, if exists
   * \return true on success
   */
...
private:
  /**
   * The routing table （路由表条目中“目的地址”和“路由表条目”的映射）
   * Ipv4Address = RoutingTableEntry.GetDestination ()
   */
