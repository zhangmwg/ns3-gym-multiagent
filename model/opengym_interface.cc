...
OpenGymInterface::Get (uint32_t port)
{
  ...
  NS_LOG_UNCOND ("OpenGymInterface Get port: " << port);
  return *DoGet (port);
}
...
{
  NS_LOG_FUNCTION (this);
  // NS_LOG_UNCOND("OpenGymInterface SetGetObservationCb");
  m_obsCb = cb;
}
...
{
  NS_LOG_FUNCTION (this);
  // NS_LOG_UNCOND("OpenGymInterface SetGetRewardCb");
  m_rewardCb = cb;
}
...
{
  NS_LOG_FUNCTION (this);
  // NS_LOG_UNCOND("OpenGymInterface SetGetExtraInfoCb");
  m_extraInfoCb = cb;
}
...
{
  NS_LOG_FUNCTION (this);
  // NS_LOG_UNCOND("OpenGymInterface SetExecuteActionsCb");
  m_actionCb = cb;
}
...
OpenGymInterface::Init()
{
  ...
  NS_LOG_UNCOND("\nSimulation process id: " << ::getpid() << " (parent (waf shell) id: " << ::getppid() << ")");
  ...
  if (stopSim) {
    ...
    NS_LOG_UNCOND("OpenGymInterface ---Stop requested: " << stopSim);
...
{
  NS_LOG_FUNCTION (this);
  // NS_LOG_UNCOND("OpenGymInterface NotifyCurrentState.");

  if (!m_initSimMsgSent) {
    // NS_LOG_UNCOND("OpenGymInterface NotifyCurrentState m_initSimMsgSent: False.");
    // NS_LOG_UNCOND("Init.");
    Init();
  }
...
{
  NS_LOG_FUNCTION (this);
  // NS_LOG_UNCOND("OpenGymInterface IsGameOver.");
  bool gameOver = false;
...
OpenGymInterface::GetActionSpace()
{
  ...
  if (!m_actionSpaceCb.IsNull())
  {
    ...
    // NS_LOG_UNCOND("actionSpace:" << actionSpace);
  }
  return actionSpace;
}
...
OpenGymInterface::GetObservationSpace()
{
  ...
  if (!m_observationSpaceCb.IsNull())
  {
    ...
    // NS_LOG_UNCOND("obsSpace:" << obsSpace);
  }
  return obsSpace;
}
...
{
  NS_LOG_FUNCTION (this);
  NS_LOG_INFO("OpenGymInterface Notify.");
